...
const Comment: React.FC<CommentProps> = ({
  ...
  return (
    <div className="w-full h-30 p-5 rounded-lg bg-white border border-gray-300  flex space-x-4 items-start justify-between">
      ...
      <div className="flex flex-col space-y-2 float-right text-sm">
        <div className="w-full">
          {/* 관리자나 작성자가 들어올 경우 보여지기 */}
          {admin === "관리자" ? (
            <ul className="flex text-xs space-x-1.5 text-gray-400">
              <li className="text-black hover:text-blue-500 cursor-pointer">
                수정
              </li>
              <li>|</li>
              <li className="text-black hover:text-red-500 cursor-pointer">
                삭제
              </li>
            </ul>
          ) : (
